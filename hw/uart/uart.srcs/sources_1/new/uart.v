module uart #(
  ...
  // Transmitter (TX) internal signals
  ...
  reg rx_receiving;
  reg [1:0] rx_sync;  // Input synchronizer to avoid metastability

  // Synchronize RX input to system clock domain
  always @(posedge clk)
  begin
  ...
  begin
    ...
    begin
      ...
      // ------------------ Receiver Logic ------------------
      data_out_valid <= 0; // Default: no new data
      if (!rx_receiving && rx_sync[1] == 0)
      begin
        // Detect start bit
        rx_receiving <= 1;
        rx_clk_counter <= BIT_PERIOD / 2; // Sample in the middle of bit
        rx_bit_index <= 0;
      end
      ...
      begin
        ...
        begin
          ...
          if (rx_bit_index < DATA_WIDTH)
          begin
            // Sample and shift in received bits
